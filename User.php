...
$(document).ready(function(e){
	var main = "UserData.php";	var mainForm = "UserForm.php";
	$('#loaderImage').show();
	...
	$(".tambahUser").click(function(){
		...
		$('.show-hide').hide(); 
		// $('#data_user').hide();
		$("#form_user").fadeIn(function(){		
			$('#loaderImage').hide(); 
		}).load(mainForm);
	});
		//$("#form_user").load(mainForm, function(){ $('#loaderImage').hide(); });
	$("#submit_result").hide();
	$("#delete_result").hide();
	$('.halaman').live("click", function(event){
		...
		$.post(main, {halaman: kd_hal} ,function(data) {
				$("#data_user").html(data).show(function(){ $('#loaderImage').hide(); });//.show()
		});
	});
	$('input:text[name=pencarian]').on('input',function(e){
		...
		if(v_cari!="") {
			$.post(main, {cari: v_cari} ,function(data) {
				$("#data_user").html(data).show(function(){ $('#loaderImage').hide(); });
			});
		} else {
			$("#data_user").load(main,function(){ $('#loaderImage').hide(); });
		}
	});
	$('.hapus').live("click", function(){
		var url = "UserSimpan.php";
		var id_parts = $(this).closest("tr").attr("id");
		var answer = confirm("Apakah anda ingin mengghapus data ini?");
		if (answer) {
			$('#loaderImage').show();
			$.post(url, {hapus: id_parts} ,function() {
				$("#form_user").load(mainForm, function(){ 
				$("#data_user").load(main, function(){ $('#loaderImage').fadeOut(); });
			});
...
<div class="header show-hide">
    ...
    <h1 class="page-title">
		<div class="btn-toolbar">
			<button class="btn btn-primary tambahUser"><i class="icon-plus"></i> Tambah</button>
