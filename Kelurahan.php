...
<script>
		...
		$(".kecamatan_menu").removeClass('active');
		$(".kelurahan_menu").addClass('active');
		...
		$("#submit_result").hide();
		$("#delete_result").hide();		
		$("#data_kelurahan").load(main, function(){ 
			$('#loaderImage').hide(); 
			$("#submit_result").hide();
			$("#delete_result").hide();
		});
		$(".tambah").click(function(){
			...
			$('.show-hide').hide(); 
			//$('#data_kelurahan').hide();
			$("#form_kelurahan").fadeIn(1, function(){
		...
		$('#loaderImage').show();
			...
			$.post(main, {halaman: kd_hal} ,function(data) {
				$("#data_kelurahan").html(data).show(function(){ $('#loaderImage').hide(); });//.show()
			});
		});
		$('input:text[name=pencarian]').on('input',function(e){
			...
			if(v_cari!="") {
				$.post(main, {cari: v_cari} ,function(data) {
					$("#data_kelurahan").html(data).show(function(){ $('#loaderImage').hide(); });
				});
			} else {
				$("#data_kelurahan").load(main,function(){ $('#loaderImage').hide(); });
			}
		});
		$('.hapus').live("click", function(){
			var url = "KelurahanSimpan.php";
			var id_kecamatan = $(this).closest("tr").attr("id");
			var answer = confirm("Apakah anda ingin mengghapus data ini?");
			if (answer) {
			$('#loaderImage').show();
				$.post(url, {hapus: id_kecamatan} ,function() { 
					$("#data_kelurahan").load(main, function(){ $('#loaderImage').fadeOut(); });
...
<div id="delete_result" class="alert alert-success">
	...
	<div class="row-fluid">
		<div class="well">
			<div id="form_kelurahan"></div>
			<div id="data_kelurahan"></div>
		</div>            
	</div>
</div>
